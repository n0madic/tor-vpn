...
| `crates/state/` | `VpnState` struct, `VpnStatus` enum, `is_tor_vpn_process()`, state file persistence (`load`/`save`/`delete_file`), `get_status()`, `uptime_from_state()`, platform defaults (`default_state_file()`, `default_socket_path()`, `default_tor_cache_dir()`) |
...
| `crates/routing/` | `NetworkController` — route management via `route_manager` crate (native OS APIs), default gateway detection, guard IP bypass (/32 IPv4, /128 IPv6), catch-all (0/1+128/1), DNS config. Submodules: `dns` (platform DNS config), `guard` (guard relay detection via netstat2, `run_guard_monitor`), `cleanup` (`cleanup_orphaned()`, `restore_dns_if_ours()` for SIGKILL recovery) |
...
- System DNS override opt-in via `--override-dns`: sets DNS to TUN-derived IP (tun_addr+1) to prevent browser DoH upgrades (required for .onion interception). Off by default
...
- Windows: TUN via wintun.dll (standard DLL search: exe dir, System32, PATH), routes via `route_manager` crate (native Windows routing API), DNS set/restore via `netsh`, DNS query via `netdev` (fallback: `netsh`), gateway detection via `netdev` (native GetAdaptersAddresses), guard detection via `netstat2` crate, process check via `sysinfo` crate, admin check via `is-admin` crate (`IsUserAnAdmin` shell32)
- Windows DNS: `netsh` does NOT auto-revert on interface removal (unlike resolvectl) — explicit cleanup required even after force-kill. `dns_method = "netsh"` stored in state file
...
- UI privilege escalation: platform-native — macOS: `osascript` "do shell script ... with administrator privileges", Linux: `pkexec` (PolicyKit), Windows: `Start-Process -Verb RunAs` (UAC). Daemon launched detached with stdout/stderr redirected to log file
