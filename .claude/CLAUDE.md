...
- `--log-file <PATH>` (global, env: `TOR_VPN_LOG_FILE`): write logs to file instead of stderr, with ANSI colors disabled. Used by UI to capture daemon output without shell redirection. CLI `--log-level`/`--log-file` override ALL config file `Log` directives (produces a single target)
- Multi-target logging: `ConfigFile` stores `log_targets: Vec<LogTarget>` (each with `level: String` + `destination: LogDestination`), `ResolvedConfig` has `log_targets: Vec<LogTarget>`. `init_logging` builds a layered `tracing_subscriber::Registry` with per-layer `EnvFilter`, enabling simultaneous logging to stderr + file at different levels (e.g., `Log info stderr` + `Log debug file /var/log/tor-vpn.log`). Empty `log_targets` vec means "not specified" (falls back to `info` stderr). `LogDestination::File` uses `safe_create_file()` per target; failed file opens are warned and skipped. If all targets fail, falls back to stderr at `info` level
